...
/**
 ** This great AVL tree implementation was stolen from OpenSolaris kernel.
 ** It is slightly modified for use in Phlox Microkernel.
 **/
 ...
 *    must constantly translate between "avl_tree_node_t *" and containing
 ...
 *    avl_tree_insert()/_add()/_remove()/avl_tree_insert_here() require some
 *    kind of exclusive write lock. Other operations require a read lock.
 *
 ...
 *    there is an explicit "parent" link in the avl_tree_node_t.
 *
 ...
 *
 *  - The avl_tree_index_t is an opaque "cookie" used to find nodes at or
 *    adjacent to where a new value would be inserted in the tree.
 */
...
#define AVL_DATA2NODE(d, o)  ((avl_tree_node_t *)((uintptr_t)(d) + (o)))
 ...
 * -2 or +2.
 */
static int avl_tree_rotation(avl_tree_t *tree, avl_tree_node_t *node, int balance)
{
    int left = !(balance < 0);  /* when balance = -2, left will be 0 */
    int right = 1 - left;
    int left_heavy = balance >> 1;
    int right_heavy = -left_heavy;
    avl_tree_node_t *parent = AVL_XPARENT(node);
    avl_tree_node_t *child = node->avl_child[left];
    avl_tree_node_t *cright;
    avl_tree_node_t *gchild;
    avl_tree_node_t *gright;
    avl_tree_node_t *gleft;
 ...
 * otherwise next node
 */
void *avl_tree_walk(avl_tree_t *tree, void *oldnode, int left)
{
    size_t off = tree->avl_offset;
    avl_tree_node_t *node = AVL_DATA2NODE(oldnode, off);
    int right = 1 - left;
    int was_child;
                     ...
                     size_t size, size_t offset)
{
    ...
    ASSERT(compare);
    ASSERT(size > 0);
    ASSERT(size >= offset + sizeof (avl_tree_node_t));
 ...
 *  *where (if where.node not NULL) is set to indicate the insertion point
 *  "void *" of the found tree node
 */
void *avl_tree_find(avl_tree_t *tree, void *value, avl_tree_index_t *where)
{
    ...
    for (node = tree->avl_root; node != NULL;
        ...
        diff = tree->avl_compare(value, AVL_NODE2DATA(node, off));
...
/*
 * Insert a new node into an AVL tree at the specified (from avl_tree_find()) place.
 *
 * Newly inserted nodes are always leaf nodes in the tree, since avl_tree_find()
 * searches out to the leaf positions.  The avl_tree_index_t indicates the node
 * which will be the parent of the new node.
 *
 * After the node is inserted, a single rotation further up the tree may
 * be necessary to maintain an acceptable AVL balance.
 */
void avl_tree_insert(avl_tree_t *tree, void *new_data, avl_tree_index_t where)
{
    avl_tree_node_t *node;
    avl_tree_node_t *parent = where.node;
    int old_balance;
    int new_balance;
    int which_child = where.child;
    size_t off = tree->avl_offset;
     ...
     */
    (void) avl_tree_rotation(tree, node, new_balance);
}


/*
 * Insert "new_data" in "tree" in the given "direction" either after or
 * before (AVL_TREE_AFTER, AVL_TREE_BEFORE) the data "here".
 *
 * Insertions can only be done at empty leaf points in the tree, therefore
 * if the given child of the node is already present we move to either
 * the AVL_TREE_PREV or AVL_TREE_NEXT and reverse the insertion direction.
 * Since every other node in the tree is a leaf, this always works.
 *
...
{
    avl_tree_node_t  *node;
    avl_tree_index_t index;
    int child = direction;  /* rely on AVL_TREE_BEFORE == 0, AVL_TREE_AFTER == 1 */
#ifdef __DEBUG__
    int diff;
#endif
    ...
    ASSERT(here != NULL);
    ASSERT(direction == AVL_TREE_BEFORE || direction == AVL_TREE_AFTER);
...
#ifdef __DEBUG__
    diff = tree->avl_compare(new_data, here);
    ASSERT(-1 <= diff && diff <= 1);
    ASSERT(diff != 0);
...
#ifdef __DEBUG__
            diff = tree->avl_compare(new_data,
    ...
    avl_tree_insert(tree, new_data, index);
}
...
{
    avl_tree_node_t *node = where.node;
    int child = where.child;
    void *data;
    size_t off = tree->avl_offset;
    ...
    return (avl_tree_walk(tree, data, direction));
}
 ...
 */
void *avl_tree_add(avl_tree_t *tree, void *new_node)
{
    avl_tree_index_t where;

    if (avl_tree_find(tree, new_node, &where) != NULL)
        return (NULL);

    avl_tree_insert(tree, new_node, where);
 ...
 * avl_tree_rotation() is used to detect when a subtree did not change overall
 * height due to a rotation.
 */
void *avl_tree_remove(avl_tree_t *tree, void *data)
{
    avl_tree_node_t *delete;
    ...
    /* ensure the node is in tree */
    if (!data || avl_tree_find(tree, data, NULL) != data)
        return NULL;
    ...
    do {
        ...
        if (new_balance == 0)
            AVL_SETBALANCE(node, new_balance);
        else if (!avl_tree_rotation(tree, node, new_balance))
            break;
    } while (parent != NULL);
 ...
 *
 * The cookie is really an avl_tree_node_t to the current node's parent and
 * an indication of which child you looked at last.
 *
 * On input, a cookie.child value of 1 indicates the tree is done.
 */
void *avl_tree_destroy_nodes(avl_tree_t *tree, avl_tree_cookie_t *cookie)
{
     ...
     */
    if (cookie->node == NULL && cookie->child == 0) {
        first = avl_tree_first(tree);
